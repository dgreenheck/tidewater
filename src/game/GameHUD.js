...
export class GameHUD {
	...
	constructor( ui, game ) {
		...
		this.purse = h( 'div', 'gm-purse tw-glass', `<span class="gm-money">$0</span><span class="gm-cooler"><span class="gm-cooler-label">${ t( 'hud.cooler' ) }</span><span class="gm-cooler-bar"><span></span></span><span class="gm-cooler-kg">0 / 30 kg</span></span><span class="gm-gauge gm-fuel"><span class="gm-fuel-label">${ t( 'hud.fuel' ) }</span><span class="gm-cooler-bar gm-fuel-bar"><span></span></span><b class="gm-fuel-l">40 L</b></span><span class="gm-gauge gm-sonar"><span class="gm-sonar-label">${ t( 'hud.sonar' ) }</span><b class="gm-sonar-d">0 m</b><span class="gm-sonar-dots"></span></span>` );
		this.fuelEl = this.purse.querySelector( '.gm-fuel' );
		this.fuelLabel = this.purse.querySelector( '.gm-fuel-label' );
		...
		this.sonarLabel = this.purse.querySelector( '.gm-sonar-label' );
		...
		this.fight = h( 'div', 'gm-fight tw-glass', `
			<div class="gm-fight-head"><span class="gm-fight-call">${ t( 'hud.fishOn' ) }</span><span class="gm-fight-dist">0 m</span></div>
			<div class="gm-tension"><span class="gm-band"></span><span class="gm-danger"></span><span class="gm-needle"></span></div>
			<div class="gm-stamina"><span class="gm-stam-label">${ t( 'hud.fish' ) }</span><span class="gm-stamina-bar"><span></span></span></div>` );
		...
		this.fStamLabel = this.fight.querySelector( '.gm-stam-label' );
	...
	refresh() {
		...
		const kg = s.holdKg;
		this.coolerLabel.textContent = s.upgrades.hold > 0 ? t( 'hud.hold' ) : t( 'hud.cooler' );
		if ( this.fuelLabel ) this.fuelLabel.textContent = t( 'hud.fuel' );
		if ( this.sonarLabel ) this.sonarLabel.textContent = t( 'hud.sonar' );
		if ( this.fStamLabel ) this.fStamLabel.textContent = t( 'hud.fish' );
	...
	update( { fight, casting, power, bite, aiming, fuel = null, sonar = null } ) {
		...
		if ( fight ) {
			...
			let call = t( 'hud.fight.reelIn' ), cls = '';
			if ( fight.tension > 0.88 ) { call = t( 'hud.fight.easeOff' ); cls = 'is-warn'; }
			else if ( fight.surge > 0.55 ) { call = t( 'hud.fight.running' ); cls = 'is-warn'; }
			else if ( fight.tension < 0.15 ) { call = t( 'hud.fight.slack' ); cls = 'is-warn'; }
			else if ( fight.tension >= fight.band[ 0 ] && fight.tension <= fight.band[ 1 ] ) { call = t( 'hud.fight.goodPressure' ); cls = 'is-good'; }
	...
	showCatch( info, ms = 9000 ) {
		...
		const badge = info.record ? `<span class="gm-badge is-record">★ ${ t( 'catch.newRecord' ) }</span>`
			: info.newSpecies ? `<span class="gm-badge is-new">${ t( 'catch.newSpecies' ) }</span>` : `<span class="gm-badge is-plain">${ t( 'catch.catch' ) }</span>`;
		let note;
		const containerName = this.game.state.upgrades.hold > 0 ? t( 'hud.hold' ).toLowerCase() : t( 'hud.cooler' ).toLowerCase();
		if ( ! info.kept ) note = `<div class="gm-catch-note is-warn">${ t( 'catch.noRoom', { container: containerName } ) }</div>`;
		else if ( info.record ) note = `<div class="gm-catch-note">${ t( 'catch.previousBest', { prevKg: info.prevBestKg.toFixed( 2 ), prevCm: info.prevBestCm, diffKg: ( info.kg - info.prevBestKg ).toFixed( 2 ) } ) }</div>`;
		else if ( info.newSpecies ) note = `<div class="gm-catch-note">${ t( 'catch.firstOne' ) }</div>`;
		else note = `<div class="gm-catch-note">${ t( 'catch.yourBest', { prevKg: info.prevBestKg.toFixed( 2 ), prevCm: info.prevBestCm } ) }</div>`;
		...
		const whereFish = info.kept ? ( this.game.state.upgrades.hold > 0 ? t( 'catch.inHold' ) : t( 'catch.inCooler' ) ) : t( 'catch.letGo' );
		c.innerHTML = `
			...
			<div class="gm-catch-bottom">
				<div class="gm-catch-stats">
					<div class="gm-stat"><span>${ t( 'catch.length' ) }</span><b>${ info.cm }<small>cm</small></b><i>${ inch.toFixed( 1 ) } in</i></div>
					<div class="gm-stat"><span>${ t( 'catch.weight' ) }</span><b>${ info.kg < 1 ? info.kg.toFixed( 2 ) : info.kg.toFixed( 1 ) }<small>kg</small></b><i>${ lb.toFixed( 1 ) } lb</i></div>
					<div class="gm-stat is-value"><span>${ t( 'catch.value' ) }</span><b>$${ info.value }</b><i>${ whereFish }</i></div>
				</div>
				${ note }
				<div class="gm-catch-foot">${ t( 'catch.continueHint' ) }<span class="gm-catch-timer"><span></span></span></div>
			</div>`;
	...
	renderInventory() {
		...
		const s = this.game.state;
		const rows = s.inventory.map( ( f ) => `<div class="gm-row has-cm"><span>${ FISH[ f.species ].name }${ f.record ? `<small>${ t( 'inventory.recordBadge' ) }</small>` : '' }</span><span class="gm-cm">${ f.cm ?? Math.round( fishLengthCm( f.species, f.kg ) ) } cm</span><span class="gm-kg">${ f.kg.toFixed( 2 ) } kg</span><span class="gm-val">$${ f.value }</span><button class="gm-mini" data-release="${ f.id }">${ t( 'inventory.releaseBtn' ) }</button></div>` ).join( '' );
		const logged = Object.entries( s.log ).filter( ( [ k ] ) => FISH[ k ] ).map( ( [ k, v ] ) => t( 'inventory.logRow', { name: FISH[ k ].name, count: v.count, bestKg: v.bestKg.toFixed( 2 ), bestCm: v.bestCm ?? Math.round( fishLengthCm( k, v.bestKg ) ) } ) ).join( '<br>' );
		this.inv.innerHTML = `
			<h2>${ s.upgrades.hold > 0 ? t( 'inventory.fishHold' ) : t( 'inventory.cooler' ) }</h2>
			<p class="gm-sub">${ t( 'inventory.sub', { count: s.inventory.length, holdKg: s.holdKg.toFixed( 1 ), maxKg: s.stats.holdKg, holdValue: s.holdValue } ) }</p>
			<div class="gm-list">${ rows || `<div class="gm-empty">${ t( 'inventory.empty' ) }</div>` }</div>
			${ logged ? `<div class="gm-log"><b>${ t( 'inventory.fishLog' ) }</b><br>${ logged }</div>` : '' }
			<div class="gm-foot"><span class="gm-sub">${ t( 'inventory.sellHint' ) }</span><button class="gm-btn is-ghost" data-close>${ t( 'inventory.closeBtn' ) }</button></div>`;
	...
	renderStand() {
		...
		const s = this.game.state;
		const v = this.vendor || { name: t( 'stand.defaultBuyer' ) };
		const rows = s.inventory.map( ( f ) => `<div class="gm-row has-cm"><span>${ FISH[ f.species ].name }</span><span class="gm-cm">${ f.cm ?? Math.round( fishLengthCm( f.species, f.kg ) ) } cm</span><span class="gm-kg">${ f.kg.toFixed( 2 ) } kg</span><span class="gm-val">$${ f.value }</span><button class="gm-mini" data-sell="${ f.id }">${ t( 'stand.sellBtn' ) }</button></div>` ).join( '' );
		this.stand.innerHTML = `
			<h2>${ v.name }</h2>
			<p class="gm-sub">${ s.inventory.length ? v.greeting || t( 'stand.greeting' ) : v.idle || t( 'stand.idle' ) }</p>
			<div class="gm-list">${ rows || `<div class="gm-empty">${ t( 'stand.empty' ) }</div>` }</div>
			<div class="gm-foot"><button class="gm-btn is-ghost" data-close>${ t( 'stand.leaveBtn' ) }</button><button class="gm-btn" data-all ${ s.inventory.length ? '' : 'disabled' }>${ t( 'stand.sellAllBtn', { value: s.holdValue } ) }</button></div>`;
...
GameHUD.prototype.renderShop = function () {
	...
	const rows = Object.entries( UPGRADES ).map( ( [ key, track ] ) => {
		...
		const btn = next
			...
			: `<span class="gm-have">${ t( 'shop.topOfLine' ) }</span>`;
		return `<div class="gm-shop-row"><span>${ track.name }: ${ next ? next.label : cur.label }<small>${ t( 'shop.now', { label: cur.label } ) }</small></span>${ btn }</div>`;

	} ).join( '' );
	const missing = s.stats.fuelL - s.fuelL;
	const fuelRow = `<div class="gm-shop-row"><span>${ t( 'shop.diesel' ) } · $${ FUEL_PRICE.toFixed( 2 ) } / L<small>${ t( 'shop.tank', { litres: s.fuelL.toFixed( 0 ), max: s.stats.fuelL } ) }</small></span>${ missing > 0.5 ? `<button class="gm-btn" data-fuel ${ s.money < FUEL_PRICE ? 'disabled' : '' }>${ t( 'shop.fillBtn', { cost: s.refuelCost() } ) }</button>` : `<span class="gm-have">${ t( 'shop.full' ) }</span>` }</div>`;
	this.stand.innerHTML = `
		<h2>${ v.name }</h2>
		<p class="gm-sub">${ t( 'shop.greetingSub', { greeting: v.greeting, money: s.money.toLocaleString() } ) }</p>
		<div class="gm-list">${ fuelRow }${ rows }</div>
		<div class="gm-foot"><span class="gm-sub">${ t( 'shop.upgradesNotice' ) }</span><button class="gm-btn is-ghost" data-close>${ t( 'shop.leaveBtn' ) }</button></div>`;
	this.stand.querySelector( '[data-close]' ).onclick = () => this.closeStand();
