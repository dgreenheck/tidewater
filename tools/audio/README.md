# Audio build (fishing + jet ski sounds)

Rebuilds the sounds in `public/audio/` from CC0 Freesound previews. Needs node and ffmpeg
...
# Jet ski: edit build-jetski.mjs with Freesound IDs, then:
node build-jetski.mjs                    # -> out/*.ogg + jetski-bank.json
cp out/*.ogg ../../public/audio/         # then copy the entries of fishing-bank.json / jetski-bank.json into src/audio/soundBank.js
```

- Loops (`reel_wind`, `reel_drag`, `line_strain`, `jetski_engine`, `jetski_rush`, `jetski_lap`): excerpt, equal-power crossfade at the wrap, loudness-normalised
